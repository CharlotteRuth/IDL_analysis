pro deco,filename, rotfile, dist_units, ms_units, Ecut, NOCHECK=nocheck, CENTERM = CENTERM, CENTERV = CENTERV, halo = halo
...
IF KEYWORD_SET(halo) THEN BEGIN
;*************************************
; READ THE FILES:
;**************************************
...
;*******************************************
    ...
    indsats= -1             ;indcomp(where(indcomp ge h.ngas+h.ndark))
...
;INDEX GAS,DM & STARS FROM PARTICULAR HALO
;*******************************************
    ind = lindgen(h.n) ;where(grp eq halo,comp=indcomp)
    inds = ind(where(ind ge h.ngas+h.ndark)) 
    indg = ind(where(ind lt h.ngas))
    indd = ind(where(ind ge h.ngas and ind lt h.ngas+h.ndark))
    indsats= -1 ;indcomp(where(indcomp ge h.ngas+h.ndark))
...
IF NOT KEYWORD_SET(centerm) THEN BEGIN
...
print,'Entering Loop for Eliminating Proper Motion......'
    if p eq 0 then begin
        IF NOT KEYWORD_SET(centerv) THEN BEGIN
            propx=s1[halo-1].vx
            propy=s1[halo-1].vy
            propz=s1[halo-1].vz
        ENDIF ELSE BEGIN
...
IF NOT  keyword_set(NOCHECK) THEN BEGIN
    ...
    IF ((check EQ 'y') OR (check EQ 'Y') OR (check EQ 'yes') OR (check EQ 'Yes') OR (check EQ 'YES')) THEN BEGIN
        ...
        oplot,[min(Etot),max(Etot)],[0,0],color=80
        print,'Ecut = ',Ecut
        Change = ''
...
IF (indsats[0] ne -1) THEN index[indsats] = 6
...
BD=starmass(filename,stars,inds,ms_units,index,s)
print,'Bulge/Disk: ',BD
;stop
end
