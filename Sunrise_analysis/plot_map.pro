...
pro plot_map,map,index,contour=cont,overlay=overlay,smooth_width=smooth_width,border=border,$
 ...
 time=time,bottom=bottom,nodata=nodata,$
 ...
 no_data=no_data,nearest=nearest,rescale_zoom=rescale_zoom,cbar=cbar,$
...
oxrange=get_map_xrange(map[index])
oyrange=get_map_yrange(map[index])
...
xmin=min(dxrange)-dx2 & xmax=max(dxrange)+dx2
ymin=min(dyrange)-dy2 & ymax=max(dyrange)+dy2
...
;-- If blending, copy background image into pixmap and make composite with
;   foreground image
 ...
 if wbuff then begin
  if (comptype eq 7) and exist(back) then begin
   ...
   wdelete,pix_win
   wset,cur_win
   tv,(1.-alpha)*temporary(back)+alpha*temporary(fore),true=3
   device2,decomp=decomp
   goto,clean_up
