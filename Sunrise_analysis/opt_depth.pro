...
PRO opt_depth,filename,kpcunit,massunit, outplot = outplot,overplot = overplot,guo = guo
...
maxrad = 20.0
nrad = 20
radii = findgen(nrad)/FLOAT(nrad)*maxrad
surface_z = fltarr(nrad - 1)

FOR i = 0, nrad - 2 DO BEGIN
    indgas=WHERE(rgas LT radii[i + 1] AND rgas gt radii[i] ,nindgas)
    surface_z[i] = TOTAL(g[indgas].mass*g[indgas].zmetal*massunit)/(!PI*(radii[i + 1]^2 - radii[i]^2))
END

plot,radii[0:nrad - 2] + maxrad/nrad/2.0,surface_z*0.4*5.86e-6,xtitle = 'Radius [kpc]',ytitle = 'Optical Depth',/ylog
