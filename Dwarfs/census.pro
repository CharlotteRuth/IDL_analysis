...
;            multiplot
            IF (mct eq 0) THEN BEGIN 
                IF (ploti eq 0) THEN census, files, resulution0, mass[mct],xtit = xtit[ploti], ytit = ytit, ymin = ymin[ploti] ELSE census, files, resulution1, mass[mct], xtit = xtit[ploti], ytit = ytit, ymin = ymin[ploti]  
            ENDIF ELSE BEGIN
                IF (ploti eq 0) THEN  census, files, resulution0, mass[mct], xtit = xtit[ploti], ytit = ytit, ymin = ymin[ploti] ELSE census, files, resulution1, mass[mct], xtit = xtit[ploti], ytit = ytit, ymin = ymin[ploti]     
            ENDELSE
        ENDFOR
...
pro census,files,resolutionshort,mass,xtit = xtit,ytit = ytit, title = title,ymin = ymin
col=[64,0,254]
m=2.325e5 ;Mass unit
t=1e9 ;time unit
...
polyfill,resolutions,[ymin,fltarr(n_elements(mtot))+1.,ymin],color=150
ind = where(ymin gt outmassfrac)
IF (ind[0] NE -1) THEN outmassfrac[ind]= ymin

;Accreted Matter (Purple)
polyfill,resolutions,[ymin,starfrac+accfrac+outmassfrac+bofrac,ymin],color=32
print,[0,starfrac+accfrac+outmassfrac+bofrac,0]
oplot,resolutionshort,starfrac+accfrac+outmassfrac+bofrac

;Stellar Mass fraction (Blue)
polyfill,resolutions,[ymin,starfrac+outmassfrac+bofrac,ymin],color=64
print,[0,starfrac+outmassfrac+bofrac,0]
oplot,resolutionshort,starfrac+bofrac+outmassfrac

;Blow Out fraction (Orange)
polyfill,resolutions,[ymin,outmassfrac+bofrac,ymin],color=210
print,[0,outmassfrac+bofrac,0]
oplot,resolutionshort,bofrac+outmassfrac

;Blow Away (Red)
polyfill,resolutions,[ymin,outmassfrac,ymin],color=254
print,[0,outmassfrac,0]
oplot,resolutionshort,outmassfrac
