...
FOR mct = 0,n_mass-1 DO BEGIN
    ...
    set_plot,'ps'
    device,filename=base+'results/massSpec_sfr_'+m[mct]+'_'+imf[0]+'.eps',bits_per_pixel=8,/TIMES
        !Y.style = 1
        if (mct eq 0) then $
          plot,resvalues,[10,10,10,10,10],xtitle = 'Softening Length ' + textoidl('[R_{vir}]'),ytitle=textoidl('<SFR> [M')+sunsymbol()+textoidl(' yr^{-1}]'), $
/xlog,/ylog,xrange = [1e-1,1e-4],yrange = [4e-4,0.25],$
YTICKNAME = [textoidl('10^{-4}'),textoidl('10^{-3}'),textoidl('10^{-2}'),textoidl('10^{-1}'),'1'],xtickname=[textoidl('10^{-1}'),textoidl('10^{-2}'),textoidl('10^{-3}'),textoidl('10^{-4}')] $
        else plot, resvalues, [1,1,1,1,1], xtitle = 'Softening Length ' + textoidl('[R_{vir}]'), ytitle=textoidl('<SFR> [M')+sunsymbol()+textoidl(' yr^{-1}]'),$
/xlog,/ylog,xrange = [0.1,0.0001],yrange = [6,25];,$
;xtickname=[textoidl('10^{-1}'),textoidl('10^{-2}'),textoidl('10^{-3}'),textoidl('10^{-4}')] ;,title='SFR/Gas Mass -- Total Mass: 10^'+trim(m[mct])+sunsymbol(),
        oplot,resvalues,avesfr[*,0,0],linestyle = 0,psym=-2,thick=2
        oplot,resvalues,avesfr[*,1,0],linestyle = 0,psym=-4,thick = 1
        oplot,resvalues,avesfr[*,2,0],linestyle = 2,psym=-5,thick = 2
        oplot,resvalues,avesfr[*,3,0],linestyle = 2,psym=-6,thick = 1
        if (mct eq 0) then begin
            legend,[textoidl('10^5'),textoidl('10^4'),textoidl('10^3'),textoidl('10^2')],linestyle=[0,0,2,2],psym=[-2,-4,-5,-6],color=[0,0,0,0],thick=[2,1,2,1],/left,pspacing = 1.5*1.5,/bottom
            xyouts,[3500],[10500],[textoidl('10^{10}M')+sunsymbol()],/device,charsize = 1.5 
        endif else begin
            legend,[textoidl('10^5'),textoidl('10^4'),textoidl('10^3'),textoidl('10^2')],linestyle=[0,0,2,2],psym=[-2,-4,-5,-6],color=[0,0,0,0],thick=[2,1,2,1],/top,pspacing = 1.5*1.5,/right
