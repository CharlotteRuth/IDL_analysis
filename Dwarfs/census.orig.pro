...
device,filename=mstr+'census.eps', /color, bits_per_pixel=8
;DEVICE, DECOMPOSED = 0
loadct,39
!p.multi=[0,3,2]
...
;  mstr='11M'
  ...
  file=directories[i]+'/'+mstr+'/o'+mstr+'.00300'
  print,file
...
plot,resolutions,outmassfrac,xrange=[5e1,1e5],/xlog,xstyle=1, yrange=[zero,1],/ylog,ystyle=1, $
...
resolutions=[50,resolutions,100000]
;oplot,mtot,fltarr(n_elements(mtot))+1.,psym=-1
polyfill,resolutions,[zero,fltarr(n_elements(mtot))+1.,zero],color=150
print,[0.0,fltarr(n_elements(mtot))+1.,0.0]
;pstop
;oplot,mtot,starfrac+accfrac+outmassfrac,color=160,psym=-1
polyfill,resolutions,[zero,starfrac+accfrac+outmassfrac+bofrac,zero],color=32
...
polyfill,resolutions,[zero,outmassfrac+bofrac,zero],color=160
