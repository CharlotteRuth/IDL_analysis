...
;#
pro lmm,gal,timer=timer,distr=distr,fraction=fraction,threshold=threshold,_extra=_extra
...
if not keyword_set(distr) then distr = [0,800] 
...
;dir='/net/scratch2/fabio/h258.cosmo50cmb.1536g2bwK/h258.cosmo50cmb.1536g2bwK'
 ...
 finalgal = where(final_halo eq gal) ; restrict to virialized galaxy
  ...
  if not keyword_set(fraction) then fraction = 0.01 else fraction=fraction*1.
...
l=1
  for i =nsteps-stepsize-1,nsteps-40*stepsize,-stepsize do begin
...
readcol,statfile[i+stepsize],halo_id,ntot,ng,nstar,ndark,mvir,rvir,gmass,smass,dmass,Vmax,Rvmax,Vdisp,xc,yc,zc,vxc,vyc,vzc,ID_A,clean,reality,FORMAT='I5,L9,L9,L9,L9,F18,F12,F18,F18,F18,F11,F11,F11,F11,F11,F11,F15,F15,F15,I6,A9,A9',/preserve_null,/silent
...
;print,i,time[i],mvir[0],mvir[1],mvir[2],mvir[3],mvir[4]
...
;    endfor
    ...
    for k =0L,n_elements(prog_linked) -1 do begin
       mergeratio= mvir[prog_linked[k]-1]/mvir[0]
       if mergeratio gt 0.07 and mergeratio lt 1.0 then begin
         print,i,'  time= ',time[i],'  merger ratio=',mergeratio,Mvir[0],Mvir[prog_linked[k]-1],'smass = ',smass[0],smass[prog_linked[k]-1]
       endif
    endfor
