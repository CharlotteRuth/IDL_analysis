...
;-------------------- Make sure the halo information and gpart are for equivalent outputs
  ...
  IF (~file_test(gloc_filename)) || keyword_set(steps_debug) THEN BEGIN
...
;     IF 1 THEN BEGIN
  ...
  ENDIF ELSE BEGIN
      ...
      IF (where(indiskarr NE 0))[0] NE -1 THEN indiskarr[where(indiskarr NE 0)] = 1
  ENDELSE
;====================================================================
...
;                      -- need not have been heated by supernova)
  ...
  IF (where(gloc_temp EQ  -1))[0] NE -1 THEN gloc_temp[where(gloc_temp EQ -1)] = 3 ;Set gas that formed stars to disk
  gloc_diff = gloc_temp[*,0:nsteps - 2] - gloc_temp[*,1:nsteps - 1]
  IF (where(gloc_diff EQ  3))[0] NE -1 THEN BEGIN
...
;                       disk or becomes disk
  ...
  IF (where(gloc_temp EQ -1))[0] NE -1 THEN gloc_temp[where(gloc_temp EQ -1)] = 3 ;Set gas that formed stars to disk
  FOR j = 0, nsteps - 2 DO $
      IF (where(gloc_temp[*,j] EQ 3))[0] NE -1 THEN gloc_temp[where((gloc_temp[*,j] EQ 3)),j + 1] = 3
  gloc_diff = gloc_temp[*,0:nsteps - 2] - gloc_temp[*,1:nsteps - 1]
  IF (where(gloc_diff EQ -3))[0] EQ -1 THEN BEGIN
      istepfirst_later = []
      ipartfirst_later = []
  ENDIF ELSE BEGIN
...
;  uniqi_first2 = [where(gloc_temp[*,0] EQ 3),(istepfirst_later + 1)*npart + ipartfirst_later]
  ...
  ENDELSE
  IF istepfirst NE !NULL THEN uniqi_first = [(istepfirst + 1)*npart + ipartfirst]
  gloc_temp = [0]
  gloc_diff = [0]
...
;  gloc_temp[indcooloff,*] = -10
  ...
  IF (where(gloc_diff EQ -3))[0] NE -1 THEN BEGIN
      ...
      IF (where(ind EQ -1))[0] NE -1 THEN BEGIN
      ...
      ENDIF ELSE BEGIN
          ipart_reaccrheat = ipartfirst
          istep_reaccrheat = istepfirst
      ENDELSE
      gloc_temp = [0]
      gloc_diff = [0]    
...
;                     to being unbound from it -- NO LONGER must be heated by supernova)
  ...
  IF indcooloff[0] NE -1 THEN gloc_temp[indcooloff,*] = -10
  FOR j = 0,nsteps - 2 DO $
    IF (where((gloc_temp[*,nsteps - 1 - j] EQ 1 OR gloc_temp[*,nsteps - 1 - j] EQ 0) AND gloc_temp[*,nsteps - 2 - j] EQ 2))[0] NE -1 THEN gloc_temp[where((gloc_temp[*,nsteps - 1 - j] EQ 1 OR gloc_temp[*,nsteps - 1 - j] EQ 0) AND gloc_temp[*,nsteps - 2 - j] EQ 2),nsteps - 2 - j] = 1
  IF (where(gloc_temp EQ 1))[0] NE -1 THEN gloc_temp[where(gloc_temp EQ  1)] = 0 ;Set disk-unbound halo gas to zero
...
;  gloc_temp[indcooloff,*] = -10
  ...
  FOR j = 1, nsteps - 1 DO $
    IF (where((gloc_temp[*,j] EQ 2) AND gloc_temp[*,j - 1] EQ 0))[0] NE -1 THEN gloc_temp[where((gloc_temp[*,j] EQ 2) AND gloc_temp[*,j - 1] EQ 0),j] = 0
  IF (where(gloc_temp EQ -1))[0] NE -1 THEN gloc_temp[where(gloc_temp EQ -1)] = 3 ;Set gas that formed stars to disk
  gloc_diff = gloc_temp[*,0:nsteps - 2] - gloc_temp[*,1:nsteps - 1]
  IF (where(gloc_diff EQ -3))[0] NE -1 THEN BEGIN
      ind_accr = where(gloc_diff EQ -3) ;Select for instances particles were first out of the disk and then in it
...
;                    disk to being out of the halo -- must be heated by supernova)  
     gloc_temp = gloc
     IF indcooloff[0] NE -1 THEN gloc_temp[indcooloff,*] = -10
     IF keyword_set(ipart_eject_true) THEN BEGIN
         gloc_temp[ipart_eject_true,istep_eject_true] = 4 ;Mark ejected gas by setting to 4
         IF (where(gloc_temp EQ -1))[0] NE -1 THEN gloc_temp[where(gloc_temp EQ -1)] = 3 ;Set gas that formed stars to disk
         FOR j = 1, nsteps - 1 DO $
           IF (where((gloc_temp[*,j] EQ 1 OR gloc_temp[*,j] EQ 2) AND gloc_temp[*,j - 1] EQ 4))[0] NE -1 THEN gloc_temp[where((gloc_temp[*,j] EQ 1 OR gloc_temp[*,j] EQ 2) AND gloc_temp[*,j - 1] EQ 4),j] = 4
         gloc_diff = gloc_temp[*,0:nsteps - 2] - gloc_temp[*,1:nsteps - 1]
         IF (where(gloc_diff EQ 4))[0] NE -1 THEN BEGIN
...
;-------------------- Find earlierst step when the particle was in the disk
     ...
     IF (everindisk[0] NE -1) THEN BEGIN
