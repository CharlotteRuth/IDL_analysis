...
FOR i = 0, N_ELEMENTS(files) - 2 DO BEGIN
    ...
    z_fut = center[i + 1,1]
    scale = 1.0/(1.0 + z)
    fb_set = where(outflow_z lt z AND outflow_z ge z_fut)
    IF fb_set[0] NE -1 THEN BEGIN
        ...
        histogramp,temp.z,title = files[i],xrange = [-10,10]
        histogramp,temp.x,/overplot,color = 240
        histogramp,temp.y,/overplot,color = 50
