PRO eject_v_mass,dirs,files,halo = halo,finalstep = finalstep,colors = colors,outplot = outplot,stellarmass = stellarmass,z_cut = z_cut,symbols = symbols,formatthick = formatthick,z_colors = z_colors,gmass = gmass,accr_star = accr_star,rewrite = rewrite
...
;      vcirct[i,iz] = stat[ind].vc
   ...
   ENDFOR
   IF keyword_set(rewrite) OR NOT file_test(dirs[i] + '/grp' + halo[i] + '.eject_quant.txt') THEN $
      eject_v_mass_data,dirs[i],files[i],halo[i],z_bins,zmax,mlbin
   openr,lun,dirs[i] + '/grp' + halo[i] + '.eject_quant.txt',/get_lun
   readf,lun,allgmass_total_temp,halogmass_total_temp,diskgmass_total_temp,reheat_total_temp,reeject_rv2_total_temp,reeject_rv2_total_temp,reeject_total_temp,sfmass_total_temp,smass_accr_total_temp
   close,lun
   free_lun,lun
   ...
   readcol,dirs[i] + '/grp' + halo[i] + '.eject_quant.txt',z_bins_temp,relostmass_temp,reheatmass_temp,reheatmassr_temp,reejectmass_rv2_temp,reejectmassr_rv2_temp,reejectmass_rv5_temp,reejectmassr_rv5_temp,reejectmass_temp,reejectmassr_temp,reejectmass_cool_temp,reejectmass_coolr_temp,reexpellmass_temp,reexpellmassr_temp,reexpellmass_cool_temp,reexpellmass_coolr_temp,diskgmass_lost_temp,diskgmass_temp,diskgmass_h_temp,diskgmass_rv2_temp,diskgmass_rv5_temp,halogmass_temp,sfmassr_temp
...
IF keyword_set(outplot) THEN  device,filename = outplot + '_test.eps',/color,bits_per_pixel= 8,xsize = xsize,ysize = ysize,xoffset =  2,yoffset =  2 ELSE window, 2, xsize = xsize, ysize = ysize
plot,xmass,(diskgmass_lost[*,nz - 1] - relostmass[*,nz - 1] + smass_accr_total)/(smass + gmass),/xlog,psym = 4,xtitle = xtitle,xrange = xrange,yrange = [0,max((diskgmass_lost[*,nz - 1] - relostmass[*,nz - 1] + smass_accr_total)/(smass + gmass))]
oplot,xrange,[1,1],linestyle = 1
IF keyword_set(outplot) THEN device, /close ELSE stop
ENDIF

;------------ Expelled and Ejected Mass as a Function of Halo Mass
...
;------------ Expelled and Ejected Baryon Frac as a Function of Halo Mass
...
;------------ Fractional halo gas accreted onto disk ------------
IF keyword_set(outplot) THEN  device,filename = outplot + '_accr_frac.eps',/color,bits_per_pixel= 8,xsize = xsize,ysize = ysize,xoffset =  2,yoffset =  2 ELSE window, 2, xsize = xsize, ysize = ysize
plot,xmass,diskgmass_total/(f_bar*vmass),/ylog,/xlog,xrange = xrange,xtitle = xtitle,yrange = [6e-2,1.3],ytitle = textoidl('M/(f_B M_{vir})'),symsize = symsize,/nodata
oplot,xrange,[1,1],linestyle = 1
oplot,xmass,(diskgmass_total + smass_accr_total)/(f_bar*vmass),psym = symcat(symbol_a),color = color_a,symsize = symsize
oplot,xmass,(halogmass_total + smass_accr_total)/(f_bar*vmass),psym = symcat(9),color = color_a,symsize = symsize
;oplot,xmass,diskgmass[*,nz-1]/vmass,psym = symcat(symbol_a),color = 20,symsize = symsize ;temporarily putting reaccretion on
legend,['Accreted to disk','Accreted to halo'],psym = [symbol_a,9],color = [color_a,color_a],/bottom,/right,box = 0
IF keyword_set(outplot) THEN device, /close ELSE stop

;---------- Fraction of Unique Accretion/Re-Accretion -----
;Gets at the amount of reaccretion
IF keyword_set(outplot) THEN  device,filename = outplot + '_re_accr_frac.eps',/color,bits_per_pixel= 8,xsize = xsize,ysize = ysize,xoffset =  2,yoffset =  2 ELSE window, 2, xsize = xsize, ysize = ysize
plot,xmass,(diskgmass_total + smass_accr_total)/(diskgmass[*,nz - 1] + smass_accr_total),/xlog,xrange = xrange,xtitle = xtitle,yrange = [0,1],ytitle = textoidl('M/(M_{reaccr})'),symsize = symsize,/nodata
oplot,xmass,(diskgmass_total + smass_accr_total)/(diskgmass[*,nz - 1] + smass_accr_total),psym = symcat(symbol_a),color = 100,symsize = symsize
oplot,xmass,(diskgmass_total + smass_accr_total)/(diskgmass_lost[*,nz - 1] + smass_accr_total),psym = symcat(symbol_a),color = 120,symsize = symsize
oplot,xmass,(halogmass_total + smass_accr_total)/(halogmass[*,nz - 1] + smass_accr_total),psym = symcat(9),color = color_a,symsize = symsize
legend,['Accreted to disk','Accreted to halo'],psym = [symbol_a,9],color = [color_a,color_a],/top,/left,box = 0
IF keyword_set(outplot) THEN device, /close ELSE stop

;---------- Fraction of Accreted Material in Final Halo
IF 1 THEN BEGIN
IF keyword_set(outplot) THEN  device,filename = outplot + '_accr_z0.eps',/color,bits_per_pixel= 8,xsize = xsize,ysize = ysize,xoffset =  2,yoffset =  2 ELSE window, 2, xsize = xsize, ysize = ysize
plot,xmass,(smass + gmass_hot)/(halogmass_total + smass_accr_total),/xlog,yrange = [0.01,1],/nodata,xtitle = xtitle,ytitle = 'Baryons in Disk/Accreted',xrange = xrange,/ylog
;oplot,xmass,(smass + gmass_hot)/(halogmass_total + smass_accr_total),psym = symcat(symbols[0]),color = colore[0],symsize = symsize
;oplot,xmass,(smass + gmass_hot)/(halogmass_total + smass_accr_total),psym = symcat(sym_outline(symbols[0])),symsize = symsize
oplot,xmass,(smass + gmass)/(diskgmass_total + smass_accr_total),psym = symcat(17),color = colore[1],symsize = symsize
oplot,xmass,(smass + gmass)/(diskgmass_total + smass_accr_total),psym = symcat(sym_outline(17)),symsize = symsize
oplot,xmass,(smass + gmass)/(diskgmass_lost[*,nz - 1] + smass_accr_total),psym = symcat(sym_outline(17)),color = colore[1],symsize = symsize
;legend,['Accreted onto the Halo','Accreted onto the Disk','Re(accreted) onto the Disk'],psym = [symbols[0:1],sym_outline(symbols[1])],color = [colore[0:1],colore[1]],/left,box = 0
legend,['Fraction of unique accretions retained','Fraction of unique+recycled accretions retained'],psym = [17,sym_outline(17)],color = [colore[1],colore[1]],/right,box = 0,/bottom
IF keyword_set(outplot) THEN device, /close ELSE stop
ENDIF ELSE BEGIN
plot,xmass,(smass + gmass_hot)/(halogmass_total + smass_accr_total),/xlog,yrange = [0,1],/nodata,xtitle = xtitle,ytitle = 'Baryons at z = 0/Baryons Accreted',xrange = xrange
oplot,xmass,(diskgmass_lost[*,nz - 1] - relostmass[*,nz - 1] + smass_accr_total)/(diskgmass_total + smass_accr_total),psym = symcat(symbols[1]),color = colore[1],symsize = symsize
oplot,xmass,(diskgmass_lost[*,nz - 1] - relostmass[*,nz - 1] + smass_accr_total)/(diskgmass_total + smass_accr_total),psym = symcat(sym_outline(symbols[1])),symsize = symsize
oplot,xmass,(diskgmass_lost[*,nz - 1] - relostmass[*,nz - 1] + smass_accr_total)/(diskgmass_lost[*,nz - 1] + smass_accr_total),psym = symcat(sym_outline(symbols[1])),color = colore[1],symsize = symsize
legend,['Accreted onto the Disk','Re(accreted) onto the Disk'],psym = [symbols[1],sym_outline(symbols[1])],color = [colore[1],colore[1]],/left,box = 0
ENDELSE


;------------ Accretion in Halo vs Disk ---------------
IF keyword_set(outplot) THEN  device,filename = outplot + '_reaccrdh_frac.eps',/color,bits_per_pixel= 8,xsize = xsize,ysize = ysize,xoffset =  2,yoffset =  2 ELSE window, 2, xsize = xsize, ysize = ysize
plot,xmass,diskgmass[*,nz-1]/halogmass[*,nz-1],psym =  symcat(symbol_a),symsize = symsize,/xlog,yrange=[0.1,0.8],xrange = xrange,xtitle = xtitle,ytitle = 'Disk Mass/Halo Mass'
oplot,xmass,(smass + gmass)/(smass + gmass_hot),psym = symcat(9),symsize = symsize
oplot,xmass,diskgmass_total/halogmass_total,psym = symcat(35),symsize = symsize
legend,['At z = 0','Ever accreted','Ever (re)accreted'],psym = [9,35,symbol_a],/right,/top,box = 0
IF keyword_set(outplot) THEN device, /close ELSE stop
...
;------------ Heated Mass as a Function of Halo Mass and Time
IF keyword_set(z_cut) THEN BEGIN
    IF keyword_set(outplot) THEN  device,filename = outplot + '_heat_mass_time.eps',/color,bits_per_pixel= 8,xsize = xsize,ysize = ysize,xoffset =  2,yoffset =  2 ELSE window, 2, xsize = xsize, ysize = ysize
    plot,xmass,reheatmass[*,0],/ylog,/xlog,xrange = xrange,yrange = [5e5,2e11],xtitle = xtitle,ytitle = 'Mass of Gas Heated by SN [M' + sunsymbol() + ']',/nodata ;yrange = [1e3,1e11]
    FOR iz = 0, nz - 1 DO BEGIN
       oplot,xmass,reheatmass[*,iz],psym = symcat(symbols(4)),color  = z_colors[iz],symsize = symsize  
       oplot,xmass,reheatmass[*,iz],psym = symcat(sym_outline(symbols(4))),symsize = symsize
    ENDFOR
    ;for Rachel
    oplot,xmass,reoutflowmass[*,3],psym = symcat(14),symsize = symsize
    ;
...
;------------ Eject (0.2 R_vir) Mass as a Function of Halo Mass and Time
IF keyword_set(z_cut) THEN BEGIN
    IF keyword_set(outplot) THEN  device,filename = outplot + '_heat_mass_time.eps',/color,bits_per_pixel= 8,xsize = xsize,ysize = ysize,xoffset =  2,yoffset =  2 ELSE window, 2, xsize = xsize, ysize = ysize
    plot,xmass,reejectmass_rv2[*,0],/ylog,/xlog,xrange = xrange,yrange = [5e5,1e11],xtitle = xtitle,ytitle = 'Mass of Gas Ejected Beyond 0.2*R' + textoidl('_{vir}') + ' [M' + sunsymbol() + ']',/nodata ;yrange = [1e3,1e11]
    FOR iz = 0, nz - 1 DO BEGIN
       oplot,xmass,reejectmass_rv2[*,iz],psym = symcat(symbols[2]),color  = z_colors[iz],symsize = symsize  
       oplot,xmass,reejectmass_rv2[*,iz],psym = symcat(sym_outline(symbols[2])),symsize = symsize
    ENDFOR
    legend,'z = ' + string(z_bins_legend,format = '(A3)') + ' ',psym = ex_psym,color  = z_colors,/right,/bottom,box = 0
    IF keyword_set(outplot) THEN device, /close ELSE stop    
ENDIF

;------------ Eject (0.5 R_vir) Mass as a Function of Halo Mass and Time
IF keyword_set(z_cut) THEN BEGIN
    IF keyword_set(outplot) THEN  device,filename = outplot + '_heat_mass_time.eps',/color,bits_per_pixel= 8,xsize = xsize,ysize = ysize,xoffset =  2,yoffset =  2 ELSE window, 2, xsize = xsize, ysize = ysize
    plot,xmass,reejectmass_rv5[*,0],/ylog,/xlog,xrange = xrange,yrange = [5e5,1e11],xtitle = xtitle,ytitle = 'Mass of Gas Ejected Beyond 0.5*R' + textoidl('_{vir}') + ' [M' + sunsymbol() + ']',/nodata ;yrange = [1e3,1e11]
    FOR iz = 0, nz - 1 DO BEGIN
       oplot,xmass,reejectmass_rv5[*,iz],psym = symcat(symbols[3]),color  = z_colors[iz],symsize = symsize  
       oplot,xmass,reejectmass_rv5[*,iz],psym = symcat(sym_outline(symbols[3])),symsize = symsize
    ENDFOR
    legend,'z = ' + string(z_bins_legend,format = '(A3)') + ' ',psym = ex_psym,color  = z_colors,/right,/bottom,box = 0
    IF keyword_set(outplot) THEN device, /close ELSE stop    
ENDIF
...
oplot,xmass, reejectmass_cool[*,nz - 1]/relostmass[*,nz - 1],psym = symcat(sym_outline(symbols[1])),color = colore[1],symsize = symsize
...
oplot,xmass, reejectmass_rv2[*,nz - 1]/diskgmass_lost[*,nz - 1],psym = symcat(symbols[2]),color = colore[2],symsize = symsize
oplot,xmass, reejectmass_rv2[*,nz - 1]/diskgmass_lost[*,nz - 1],psym = symcat(sym_outline(symbols[2])),symsize = symsize
;oplot,xmass, reejectmass_rv5[*,nz - 1]/diskgmass_rv5[*,nz - 1],psym = symcat(symbols[3]),color = colore[3],symsize = symsize
;oplot,xmass, reejectmass_rv5[*,nz - 1]/diskgmass_rv5[*,nz - 1],psym = symcat(sym_outline(symbols[3])),symsize = symsize
...
;------------ Fraction of Gas Mass Ejected
IF keyword_set(outplot) THEN  device,filename = outplot + '_frac_gas_ejectedrv2_mass.eps',/color,bits_per_pixel= 8,xsize = xsize,ysize = ysize,xoffset =  2,yoffset =  2 ELSE window, 2, xsize = xsize, ysize = ysize
plot,xmass,reejectmass_rv2[*,nz - 1]/diskgmass_lost[*,nz - 1],/xlog,xrange = xrange,yrange = [0,0.5],xtitle = xtitle,ytitle = 'Fraction of Gas Mass Ejected Beyond 0.2*R' + textoidl('_{vir}'),/nodata;yrange = [0,1]
oplot,xmass, reejectmass_rv2[*,nz - 1]/diskgmass_lost[*,nz - 1],psym = symcat(symbols[2]),color = colore[2],symsize = symsize
oplot,xmass, reejectmass_rv2[*,nz - 1]/diskgmass_lost[*,nz - 1],psym = symcat(sym_outline(symbols[2])),symsize = symsize
IF keyword_set(outplot) THEN device, /close ELSE stop

;------------ Fraction of Gas Mass Ejected
IF keyword_set(outplot) THEN  device,filename = outplot + '_frac_gas_heat_mass.eps',/color,bits_per_pixel= 8,xsize = xsize,ysize = ysize,xoffset =  2,yoffset =  2 ELSE window, 2, xsize = xsize, ysize = ysize
plot,xmass,reheatmass[*,nz - 1]/diskgmass_lost[*,nz - 1],/xlog,xrange = xrange,yrange = [0.7,1],xtitle = xtitle,ytitle = 'Fraction of Gas Mass Heated by SN',/nodata;yrange = [0,1]
oplot,xmass, reheatmass[*,nz - 1]/diskgmass_lost[*,nz - 1],psym = symcat(symbols[4]),color = colore[4],symsize = symsize
oplot,xmass, reheatmass[*,nz - 1]/diskgmass_lost[*,nz - 1],psym = symcat(sym_outline(symbols[4])),symsize = symsize
IF keyword_set(outplot) THEN device, /close ELSE stop

;------------ Fraction of Gas Mass Ejected
IF keyword_set(outplot) THEN  device,filename = outplot + '_frac_gas_ejectedrv5_mass.eps',/color,bits_per_pixel= 8,xsize = xsize,ysize = ysize,xoffset =  2,yoffset =  2 ELSE window, 2, xsize = xsize, ysize = ysize
plot,xmass,reejectmass_rv5[*,nz - 1]/diskgmass_lost[*,nz - 1],/xlog,xrange = xrange,yrange = [0,0.35],xtitle = xtitle,ytitle = 'Fraction of Gas Mass Ejected Beyond 0.5*R' + textoidl('_{vir}'),/nodata;yrange = [0,1]
oplot,xmass, reejectmass_rv5[*,nz - 1]/diskgmass_lost[*,nz - 1],psym = symcat(symbols[3]),color = colore[3],symsize = symsize
oplot,xmass, reejectmass_rv5[*,nz - 1]/diskgmass_lost[*,nz - 1],psym = symcat(sym_outline(symbols[3])),symsize = symsize
IF keyword_set(outplot) THEN device, /close ELSE stop
...
plot,xmass,reejectmass[*,nz - 1]/reheatmass[*,nz - 1],ytitle = textoidl('M/M_{heated}'),xtitle = xtitle,yrange = [0,0.6],xrange = xrange,psym = symcat(18),/nodata,/xlog
loadct,0
...
FOR i = 0,n_elements(xmass) - 1 DO oplot,[xmass[i],xmass[i]],[0,reejectmass[i,nz - 1]/reheatmass[i,nz - 1]],color = colore[1],thick = bar_thick
FOR i = 0,n_elements(xmass) - 1 DO oplot,[xmass[i],xmass[i]],[0,reexpellmass[i,nz - 1]/reheatmass[i,nz - 1]],color = colore[0],thick = bar_thick
oplot,xmass,reejectmass[*,nz - 1]/reheatmass[*,nz - 1],psym = symcat(symbols[1]),symsize = symsize,color = colore[1]
oplot,xmass,reejectmass[*,nz - 1]/reheatmass[*,nz - 1],psym = symcat(sym_outline(symbols[1])),symsize = symsize
oplot,xmass,reexpellmass[*,nz - 1]/reheatmass[*,nz - 1],psym = symcat(symbols[0]),symsize = symsize,color = colore[0]
oplot,xmass,reexpellmass[*,nz - 1]/reheatmass[*,nz - 1],psym = symcat(sym_outline(symbols[0])),symsize = symsize
...
oplot,xmass,reexpellmass[*,nz - 1]/reheatmass[*,nz - 1],symsize = symsize,psym = symcat(symbol_a),color = colore[4]
oplot,xmass,reexpellmass[*,nz - 1]/reejectmass_rv2[*,nz - 1],symsize = symsize,psym = symcat(36),color = [colore[2]]
;oplot,xmass,reexpellmass[*,nz - 1]/relostmass[*,nz - 1],symsize = symsize,psym = symcat(symbol_a),color = colore[5]
legend,['Heated','Unbound from disk','Beyond R' + textoidl('_{vir}')],psym = [symbol_a,9,36],color = [colore[4],colore[1],colore[2]],box = 0
...
oplot,xarr,reejectmass_rv2[*,nz - 1]/sfmass_total,psym = symcat(symbols[2]),color = colore[2],symsize = symsize
...
;    oplot,xarr,reheatmass[*,nz - 1]/sfmass_total,psym = symcat(sym_outline(symbols[4])),symsize = symsize
    FOR iz = 0, nz - 1 DO BEGIN
       ...
       oplot,xarrt[*,iz],reheatmassr[*,iz]/sfmassr[*,iz],psym = symcat(sym_outline(symbols[4])),symsize = symsize*0.6
    ENDFOR
    fits_eject = robust_linefit( alog10(xarrt), alog10(reheatmassr/sfmassr), reejectmass_fit, sigma_eject )
    print,'Heated: ',fits_eject
    oplot,mlrange,10^fits_eject[0]*mlrange^(fits_eject[1]),linestyle = 0,psym = -3,thick = thicks[0]
    legend,'z = ' + string(z_bins_legend,format = '(A3)'),psym = ej_psym,color  = z_colors,/right,/top,box = 0
    IF keyword_set(outplot) THEN device, /close ELSE stop    
 ENDIF
...
oplot,xmass,reheatmass[*,nz - 1]/sfmass_total,psym = symcat(symbols[4]),color = colore[4],symsize = symsize
oplot,xmass,reheatmass[*,nz - 1]/sfmass_total,psym = symcat(sym_outline(symbols[4])),symsize = symsize
legend,['Gas heated from disk'],psym = [symbols[4]],color = [colore[4]],symsize = symsize,/left,/bottom,box = 0
;legend,['Gas Mass Expelled/Stellar Mass Formed','Gas Mass Ejected/Stellar Mass Formed'],psym = symbols,color = colore,/bottom,/left,box=0
multiplot
...
;------------ Fraction of Gas Mass Accreted Over Time
IF keyword_set(z_cut) THEN BEGIN
    IF keyword_set(outplot) THEN  device,filename = outplot + '_frac_gas_accr.eps',/color,bits_per_pixel= 8,xsize = xsize,ysize = ysize,xoffset =  2,yoffset =  2 ELSE window, 2, xsize = xsize, ysize = ysize
    plot,xmass,diskgmass[*,nz - 1]/diskgmass[*,nz - 1],/xlog,xrange = xrange,yrange = [0,1],xtitle = xtitle,ytitle = 'Fraction of Gas Mass Accreted',/nodata; yrange = [0,1]
;    FOR iz = 0, nz - 1 DO oplot,xmass,diskgmass[*,iz]/diskgmass[*,nz - 1],psym = symcat(z_psym[iz]),color  = z_colors[iz]
;    FOR iz = 0, nz - 1 DO oplot,xmass,diskgmass[*,iz]/diskgmass[*,nz - 1],psym = symcat(sym_outline(z_psym[iz]))
    FOR iz = 0, nz - 1 DO BEGIN
       oplot,xmass,diskgmass[*,iz]/diskgmass[*,nz - 1],psym = symcat(z_psym[iz]),color  = z_colors[iz]
       oplot,xmass,diskgmass[*,iz]/diskgmass[*,nz - 1],psym = symcat(sym_outline(z_psym[iz]))
    ENDFOR
