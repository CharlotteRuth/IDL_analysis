...
;-
function BETTER_HIST2D, V1, V2, Weight, BINSIZE1=Binsize1, BINSIZE2=Binsize2, $
...
;   Remove data points outside MAX/MIN range
         iout = WHERE( (V1 gt Max1) or (V1 lt Min1) or $
                       (V2 gt Max2) or (V2 lt Min2), nout, complement = iin )
...
;   Define histogram parameters
         d1   = double(Binsize1)
         d2   = double(Binsize2)
         w1   = double(double(Max1) - double(Min1))
         w2   = double(double(Max2) - double(Min2))
         I1m  = floor(float(w1)/float(d1))
         I2m  = floor(float(w2)/float(d2))
         n1   = I1m ;+ 1L
         n2   = I2m ;+ 1L

;   Take care of OUTPUT KEYWORDS
         Omax1 = double(Max1) & Omax2 = double(Max2)
         Omin1 = double(Min1) & Omin2 = double(Min2)
