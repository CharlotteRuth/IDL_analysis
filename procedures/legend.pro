...
pro legend, items, BOTTOM_LEGEND=bottom, BOX = box, CENTER_LEGEND=center, $
    ...
    OUTLINE_COLOR = outline_color, FONT = font, BTHICK = bthick, linsize = linsize, CTABLES = CTABLES
;
;       =====>> HELP
;
compile_opt idl2
...
;; extra by djseed .. psym=88 means use the already defined usersymbol
 ...
 if psym[i] EQ 10 then $
    ...
    if psym[i] le 0 then $
       if abs(psym[i]) GT 8 then p_sym = -1*symcat(abs(psym[i]),thick = thick[i]) else p_sym= psym[i] $
    else $
       if psym[i] GT 8 then p_sym = symcat(psym[i],thick = thick[i]) else p_sym= psym[i]
 endelse
