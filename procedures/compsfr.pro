...
print,file
    ...
    sfr,s,sfh=sfh,maxt=maxt,massu=m,time=t,tarray=tarray,xmargin=xmargin, $
       ymargin=ymargin,over=j,line=j
    avesf[j] = mean(sfh)
endfor
    legend,['1 M particles: '+string(avesf[0],format='(e8.2)')+' M'+ $
      sunsymbol()+' yr!u-1!n','100k parts: '+ $
      string(avesf[1],format='(e8.2)')+' M'+sunsymbol()+' yr!u-1!n'], $
      line=[0,1],/right
ppsclose
stop
    nbins=maxt/1e9
...
print,file
    rtipsy,file,h,g,d,s
    sfr,s,sfh=sfh,maxt=maxt,massu=m,time=t,tarray=tarray,/xlog,/lookback; $
