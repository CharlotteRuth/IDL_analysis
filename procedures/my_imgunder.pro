pro my_imgunder, z, interp=interp, help=hlp, channel=channel
;+
; NAME:
;       IMGUNDER
; PURPOSE:
...
;-------------------------------------------------------------
	...
	if !d.name ne 'PS' then begin
	  ...
	  dx = xx(1) - xx(0)
	  dy = yy(1) - yy(0)
	  tv, congrid(z, dx, dy, interp=interp), xx(0), yy(0), channel=channel
	;--------  PostScript  ----------
	endif else begin
	  ...
	  dx = xx(1) - xx(0)
	  dy = yy(1) - yy(0)
	  tv, z, xx(0), yy(0), xsize=dx, ysize=dy, /cent,channel=channel
	endelse
