...
FUNCTION selectregion_tipsy, xvec, yvec,   $
                       ...
                       OUTSIDE = outside, color = color, NOPLOT = NOPLOT, AREA = area
...
IF NOT KEYWORD_SET(noplot) THEN BEGIN
    ...
    IF KEYWORD_SET(color) THEN BEGIN
        FOR i = 0LL, N_ELEMENTS(xvec) - 1 DO oplot,[xvec[i],xvec[i]],[yvec[i],yvec[i]],psym = 3,color = color[i]
    ENDIF
ENDIF
