...
; OPTIONAL INPUTS:
;
;    DOPLOT: Set this in order to plot to the screen. If both PLOT and
;          OUTPLOT are set, it defaults to PLOT (plots to screen)
;
...
; 
;    OUTPLOT: Set to plot to a file. If both DOPLOT and OUTPLOT are set,
;             it defaults to DOPLOT (plots to screen)
;
...
pro moments, cube, header, mom0, mom1, mom2, xaxis=xaxis, $
             yaxis=yaxis, $
             doplot=doplot, threshold=threshold, outplot=outplot,$
             ...
             xrange=xrange, yrange=yrange, maxlevel = maxlevel

if not (keyword_set(cube) and keyword_set(header)) then begin
    print,"moments, cube, header, [mom0, mom1, mom2, xaxis, yaxis, $"
    print,"         doplot=doplot, threshold=threshold, outplot=outplot, $"
    print,"         nl0=nl0, nl1=nl1, nl2=nl2, name=name, $"
    print,"         maxthreshold=maxthreshold, plotthresh=plotthresh, $"
    print, "         docontour=docontour, nc0=nc0, nc1=nc1, nc2=nc2]"
    return
endif
...
;    hmom3
    ...
    if (foo.type_name eq 'INT') then begin
        if (fits eq 1) then begin
            mwrfits, mom0, outplot +'.mom0.fits', mwrheader, /create
            mwrfits, mom1,outplot + '.mom1.fits', mwrheader, /create
            mwrfits, mom2, outplot +'.mom2.fits', mwrheader, /create
        endif
    endif else begin
...
if keyword_set(outplot) or keyword_set(doplot) then begin

    if not keyword_set(doplot) then begin
        set_plot,'ps'
        device,filename=outplot + '.mom0.ps',xsize=7,ysize=5.7,/inches
    endif else begin
        set_plot,'x'
    ...
    loadct,0
    levs0 = dblarr(nl0)
    if (where(finite(mom0)))[0] ne -1 THEN BEGIN
    levs0[nl0-1] = max(mom0[where(finite(mom0))]) * .95
    IF(keyword_set(maxlevel)) THEN levs0[nl0-1] = maxlevel
    ...
    if not keyword_set(doplot) then begin 
        device,/close
        device,filename=outplot + '.mom1.ps',/color,bits=8,$
...
;    contour, mom1, xaxis, yaxis, levels=levs1, /overplot
    ...
    if not keyword_set(doplot) then begin
        device,/close
        device,filename=outplot + '.mom2.ps', /color,bits=8,$
          xsize=7, ysize=5.7, /inches, /encapsulated
    endif else stop
    ...
    levs2 = [0, 2, 5, 10, 20, 50] ; HACK ADDED BY AMS
    nlevels = 10.0
