...
pro total_line_profile, cube, header, vaxis, spectrum, w, $
                        ...
                        onepeak=onepeak, shift=shift, bin=bin,xyrange = xyrange
...
;radius = fltarr(N_ELEMENTS(xaxis),N_ELEMENTS(yaxis))
;FOR ix = 0,N_ELEMENTS(xaxis) - 1 DO $
;  FOR iy = 0,N_ELEMENTS(yaxis) - 1 DO $
;  radius[ix,iy] = SQRT(xaxis[ix]*xaxis[ix] + yaxis[iy]*yaxis[iy]);/SIN(angle)/SIN(angle)
indx = where(xaxis ge xyrange OR xaxis le -1.0*xyrange)
indy = where(yaxis ge xyrange OR yaxis le -1.0*xyrange)
;cube[where(radius gt xyrange),*] = 0
cube[indx,indy,*] = 0
stop
endif
