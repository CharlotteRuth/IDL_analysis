pro mergertree, simname, gid, zs=zs, grps=grps, tipsys=tipsys, maxz=maxz, $
  ...
  if(n_elements(grps) LE 0) then begin
    files = file_search(simname+"*/*.0????.amiga.grp")
  ...
  if(n_elements(grps) LE 0) then begin
    headers = REPLICATE(h,nfs)
    print,"Reading ",files[nfs-1]
 ;   newgroups = rbarray(files[nfs-1],/long)
    close,1
    readarr,files[nfs-1],h,newgroups,/ascii,part = 'dark'
  ...
  openw,lun,tipsys[nfs-1]+'g'+strtrim(gid,2)+'.txt',/get_lun
  paperps,file=tipsys[nfs-1]+'g'+strtrim(gid,2)+'.eps',charsize=2
...
;      oldgroups = rbarray(files[i],type='long')
      close,1
      readarr,files[i],headers[i],oldgroups,/ascii,part = 'dark'
      grps[*,i] = oldgroups
