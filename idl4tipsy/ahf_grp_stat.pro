...
IF keyword_set(munit) eq 0 then begin
   start = strpos(tipsyfile[j],'cosmo')
   IF start[0] NE -1 THEN BEGIN
      name = strmid(tipsyfile[j],start[0],7) ;5,7)
...
ENDIF
lunit = boxsize/h0
print, tipsyfile[j], ', Mass Unit = ', munit

rtipsy, tipsyfile[j], h,g,d,s
command = "ls "+tipsyfile[j]+"*halos"
spawn,command,halosfile  ; return the files without the .amiga.grp
command = "ls "+tipsyfile[j]+"*particles"
...
for i=0L, n_elements(startind)-1 do begin
  ...
  if ndark NE 0 then begin
    ...
    test2 = where(d[halo(dark_ind)].mass GT 1.50*mindm, ntest2)
...
pot_false = where(stat.n_dark lt 64, nfalse, comp=real)
...
distance = ((test_xc^2.+test_yc^2.+test_zc^2.)^0.5)  ;change from Mpc to kpc
