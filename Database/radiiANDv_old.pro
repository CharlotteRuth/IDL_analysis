...
;    temp = MIN(ABS(density  - 200.0),i200)
    ...
    IF((Where(density lt 200))[0] ne -1)  then begin 
        ...
        IF((WHERE(distancess*dkpcUnit le radii[ct,0]))[0] ne -1) THEN totalmass = TOTAL(mass[where(distances*dkpcUnit LE radii[ct,0])]) ELSE totalmass = 0
        velocity[ct,0] = SQRT(totalmass*dMsolUnit*msol*grav/radii[ct,0]/kpc)
        IF((WHERE(distancess*dkpcUnit le radii[ct,0]))[0] ne -1) THEN  stellarmass[ct,0] = TOTAL(s[WHERE(distancess*dkpcUnit le radii[ct,0])].mass)*dMsolUnit ELSE  stellarmass[ct,0] = 0
        baryonicmass[ct,0] = stellarmass[ct,0] + TOTAL(g[WHERE(distancesg*dkpcUnit le radii[ct,0]) AND g.tempg LE 40000.].mass) * dMsolUnit
...
;stop
        ...
        IF((WHERE(distancess*dkpcUnit le radii[ct,1]))[0] ne -1) THEN totalmass = TOTAL(mass[where(distances*dkpcUnit LE radii[ct,1])]) ELSE totalmass = 0
        ...
        baryonicmass[ct,1] = stellarmass[ct,1] + TOTAL(g[WHERE(distancesg*dKpcUnit le radii[ct,1]) AND g.tempg LE 40000.].mass) * dMsolUnit

        radii[ct,2] = 10./220.*velocity[ct,0]/hubble
        IF((WHERE(distancess*dkpcUnit le radii[ct,2]))[0] ne -1) THEN totalmass = TOTAL(mass[where(distances*dkpcUnit LE radii[ct,2])]) ELSE totalmass = 0
        ...
        baryonicmass[ct,2] = stellarmass[ct,2] + TOTAL(g[WHERE(distancesg*dKpcUnit le radii[ct,2]) AND g.tempg LE 40000.].mass) * dMsolUnit
...
oplot,ALOG10(velocity[*,0]),4.08-2.5*ALOG10(stellarmass[where(flag eq 1),0]/1.5) - 5*ALOG10(hubble),psym = 4,color = 50
oplot,ALOG10(velocity[*,1]),4.08-2.5*ALOG10(stellarmass[where(flag eq 1),1]/1.5) - 5*ALOG10(hubble),psym = 4,color = 100
oplot,ALOG10(velocity[*,2]),4.08-2.5*ALOG10(stellarmass[where(flag eq 1),2]/1.5) - 5*ALOG10(hubble),psym = 4,color = 240
...
oplot,velocity[*,0],baryonicmass[where(flag eq 1),0],psym = 4,color = 50
oplot,velocity[*,1],baryonicmass[where(flag eq 1),1],psym = 4,color = 100
oplot,velocity[*,2],baryonicmass[where(flag eq 1),2],psym = 4,color = 240
legend,['R200','20/220*v200/h','10/220*v200/h'],color=[50,100,240],psym = [4,4,4]
set_plot, 'ps'
